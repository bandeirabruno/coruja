...
class OcupacaoEspelhoPDF extends FPDF {
	...
	addslashes($_REQUEST['periodo']),addslashes($_REQUEST['espaco']),addslashes($_REQUEST['siglaCurso']));
...
public function obterGrade(){
	$query = sprintf("select ts.diaSemana as diaSemana ,SUBSTRING(ts.horaInicio,1,5) as horaInicio,
        SUBSTRING(ts.horaFim,1,5) as horaFim, cc.siglaDisciplina as siglaDisciplina,
	SUBSTRING(p.nome,1,16) as professor, e.nome as nome
        from TempoSemanal as ts left outer join	(Aloca a inner join Turma t
        on a.idTurma=t.idTurma and t.idPeriodoLetivo='%s' and t.tipoSituacaoTurma<>'CANCELADA'
        inner join Espaco e
...
order by diaSemana,t.turno,ts.horaInicio",
	addslashes($_REQUEST['periodo']),
        addslashes($_REQUEST['espaco']),
        addslashes($_REQUEST['siglaCurso']));
	$query= BD::mysqli_query($query);
