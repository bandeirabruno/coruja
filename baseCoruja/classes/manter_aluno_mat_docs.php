...
function selecionaCurso(){
    $con = BD::conectar();
         ...
         $fCursoQuery = BD::mysqli_query($fCurso) or die(mysqli_error($con)); 
             
         while($rowCurso=mysqli_fetch_array($fCursoQuery)){
             ...
             $idTipoCurso = $rowCurso['idTipoCurso'];
                ...
                $fLetivoQuery = BD::mysqli_query($fLetivo) or die(mysqli_error($con)); 
                    while($rowLetivo=mysqli_fetch_array($fLetivoQuery)){
              ...
              $fMatriz = "SELECT * FROM matrizcurricular WHERE siglaCurso='$siglaCurso' ORDER BY dataInicioVigencia DESC LIMIT 1";
                $fMatrizQuery = BD::mysqli_query($fMatriz) or die(mysqli_error($con)); 
                    while($rowMatriz=mysqli_fetch_array($fMatrizQuery)){
...
function formaIngresso(){ 
    $con = BD::conectar();
          ...
          $fIngressoQuery = BD::mysqli_query($fIngresso) or die(mysqli_error($con)); 
     
             while($rowIngresso=mysqli_fetch_array($fIngressoQuery)){
...
function docsEntregues(){ 
    ...
    $fCursoQuery = BD::mysqli_query($fCurso) or die(mysqli_error($con)); 
        ...
        $docEntr .= "<label>&nbsp;</label>";
            ...
            $fDocsQuery = BD::mysqli_query($fDocs) or die(mysqli_error($con)); 
                while($rowDocs=mysqli_fetch_array($fDocsQuery)){
