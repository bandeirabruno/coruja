    ...
    function insereEvento($difDatas)
        {
            ...
            for($i = 0; $i <=$difDatas;$i++ ){
	            ...
	            BD::mysqli_query($queryEvento,$con);
...
* */
    ...
    public function listaEvento($idPeriodoLetivo, $idCasoUso){
        ...
        // lista os peridos
        $lista = BD::mysqli_query("SELECT date_format(data,'%d/%m/%Y') as data,"
 ...
 public function isAlteracaoInvalida($idPeriodo,$dataInicio,$dataFim){
            ...
            $result = BD::mysqli_query($sql,$con);
         
            if($result){
            	
            	if(BD::mysqli_result($result,0,'cont')=='invalido'){
            	  $encontrou = true;
            	}
            }
            return $encontrou;
   ...
   public static function isDataInvalida($data,$dataInicialPeriodo,$dataFinalPeriodo){
            ...
            if($result){
            	...
            	if(BD::mysqli_result($result,0,'estaNoPeriodo')=='nao'){
            	  $invalido = true;
            	}
            }
            return $invalido;
 ...
 public function getEvento($seqEvento)
{
       ...
       $sqlEvento = BD::mysqli_query($queryEvento,$con);
       $qtd = mysqli_num_rows($sqlEvento);
       if($qtd > 0)
       {
           $this->setIdPeriodoLetivo(BD::mysqli_result($sqlEvento,0,'idPeriodoLetivo'));
           $this->setData(BD::mysqli_result($sqlEvento,0,'data'));
           $this->setTipoEvento(BD::mysqli_result($sqlEvento,0,'tipoEvento'));
           $this->setDescricao(BD::mysqli_result($sqlEvento,0,'descricao'));
