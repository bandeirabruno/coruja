          ...
          <tr>
        	...
        	<td>
        	    <select id="tipoEvento" name="tipoEvento" onChange="preencheDescricaoEvento(this);">
					 ...
					 <option value='INI_SOLIC_INSCR_TURMA' <?php if($evento!="") { if($evento->getTipoEvento()=='INI_SOLIC_INSCR_TURMA'){?> selected="selected" <?php }}?>>Início de solicitação de inscrição em turmas</option>
					 <option value='FIM_SOLIC_INSCR_TURMA' <?php if($evento!="") { if($evento->getTipoEvento()=='FIM_SOLIC_INSCR_TURMA'){?> selected="selected" <?php }}?>>Fim de solicitação de inscrição em turmas</option>
             			<option value='FIM_INCL_EXCL_DISCIPLINA' <?php if($evento!="") { if($evento->getTipoEvento()=='FIM_INCL_EXCL_DISCIPLINA'){?> selected="selected" <?php }}?>>Fim de inclusão e exclusão de disciplina</option>
                        <option value='FIM_TRANC_MATRICULA' <?php if($evento!="") { if($evento->getTipoEvento()=='FIM_TRANC_MATRICULA'){?> selected="selected" <?php }}?>>Fim de Trancamento de Matrícula</option>
        		</select> 
        	</td>
        </tr>
