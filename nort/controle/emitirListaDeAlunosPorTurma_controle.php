...
switch ($acao) 
{
        ...
        if( $siglaCurso !== NULL)
        {
            ...
            if( $idPeriodoLetivo !== NULL) 
            {
                ...
                $queryDisciplinas = sprintf("".
                    ...
                    $idPeriodoLetivo);
                $resultDisciplinas = BD::mysqli_query( $queryDisciplinas, $conDisciplinas);
                while( $resCC = mysqli_fetch_array( $resultDisciplinas) ) 
                {
                ...
                $queryProfessores = sprintf(
                    ...
                    mysqli_real_escape_string($con,  $siglaCurso),
                    $idPeriodoLetivo);
                $resultProfessores = BD::mysqli_query( $queryProfessores, $conProfessores);
                while( $resPro = mysqli_fetch_array( $resultProfessores) ) 
                {
    ...
    case 'buscarTurmasResultado':
        ...
        $filtroPeriodoLetivo = " AND PL.`idPeriodoLetivo` = " . mysqli_real_escape_string($con, $idPeriodoLetivo) . " ";
        $filtroCurso = " AND T.`siglaCurso` = '" . mysqli_real_escape_string($con, $siglaCurso) . "' ";

        if( !empty( $siglaDisciplina))
        {
            $filtroDisciplina = " AND T.`siglaDisciplina` = '" . mysqli_real_escape_string($con,  $siglaDisciplina) . "' ";
        }
        else
        {
            $filtroDisciplina = "";
        ...
        $result = BD::mysqli_query( $SQL_TURMAS);
...
function registrarLog( $turmas) 
{
    ...
    foreach ($turmas as $idTurma) {
        ...
        $resultPeriodoLetivo = BD::mysqli_query($SQL_PERIODO_LETIVO);
        while ($row = mysqli_fetch_array( $resultPeriodoLetivo, MYSQLI_ASSOC)) 
        {
        ...
        $resultSiglaCurso = BD::mysqli_query($SQL_PERIODO);
        while ($row = mysqli_fetch_array( $resultSiglaCurso, MYSQLI_ASSOC)) 
        {
        ...
        $SQL_NOME_DO_PROFESSOR = sprintf("select P.`nome` "
                        ...
                        . "   ) "
                        . ") ", $idTurma);
        $resultNomeProfessor = BD::mysqli_query($SQL_NOME_DO_PROFESSOR);
        while ($row = mysqli_fetch_array( $resultNomeProfessor, MYSQLI_ASSOC)) 
        {
        ...
        $SQL_NOME_DISCIPLINA = 'select CC.`siglaDisciplina`, CC.`nomeDisciplina` '
                ...
                . 'where T.`idTurma` = ' . $idTurma . ' '
                . 'and T.siglaDisciplina = CC.`siglaDisciplina`';
        $resultDisciplina = BD::mysqli_query($SQL_NOME_DISCIPLINA);
        while ($row = mysqli_fetch_array( $resultDisciplina, MYSQLI_ASSOC)) {
        ...
        $resultTurnoGrade = BD::mysqli_query($SQL_TURNO_DA_TURMA);
        while ($row = mysqli_fetch_array( $resultTurnoGrade, MYSQLI_ASSOC)) {
            $turno = $row['turno'];
...
function gerarPDF($turmas) {
    ...
    foreach ($turmas as $idTurma) 
    {
        ...
        $resultNomeProf = BD::mysqli_query($SQL_NOME_DO_PROFESSOR);
        while ($row = mysqli_fetch_array( $resultNomeProf, MYSQLI_ASSOC)) {
            $nomeDoProfessor = $row['nome'];
        }
        ...
        $SQL_NOME_DISCIPLINA = 'select CC.siglaDisciplina, CC.nomeDisciplina '
                ...
                . 'where T.idTurma = ' . $idTurma . ' '
                . 'and T.siglaDisciplina = CC.siglaDisciplina';
        $resultNomeDisciplina = BD::mysqli_query($SQL_NOME_DISCIPLINA);
        while ($row = mysqli_fetch_array( $resultNomeDisciplina, MYSQLI_ASSOC)) 
        {
            $siglaDaDisciplina = $row['siglaDisciplina'];
            $nomeDaDisciplina = $row['nomeDisciplina'];
        }
        ...
        $resultPeriodoDisciplina = BD::mysqli_query($SQL_PERIODO);
        while ($row = mysqli_fetch_array( $resultPeriodoDisciplina, MYSQLI_ASSOC)) 
        {
...
ORDER BY
    P.`nome`, MA.`matriculaAluno`',
                        $idTurma);
        $result = BD::mysqli_query($SQL_ALUNOS_NA_TURMA);
        $listaDeAlunos = array();
