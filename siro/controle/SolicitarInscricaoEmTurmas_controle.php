...
//Inserir validação de matrículas aqui 2017-2
//desativar verificação de documentos pendetes........ remover o false para voltar a funcionar.
if( TipoDocumento::obterTipoDocumentosNaoEntregues($matriculaAluno->getMatriculaAluno(), $matriculaAluno->getSiglaCurso()) && false)
{
    $documentos  = TipoDocumento::obterTipoDocumentosNaoEntregues($matriculaAluno->getMatriculaAluno(), $matriculaAluno->getSiglaCurso());

    require_once "$BASE_DIR/siro/formularios/permissao/documentoPendente.php";

    echo '<p align="center">';
        
                foreach ($documentos as $chave => $documento) {
                echo ($chave + 1)." - ". $documento->getDescricao()."<br>";
                }
...
if( !EventoPeriodoLetivo::verificaInicioInscricoes($periodoLetivo->getIdPeriodoLetivo()))
{
    require_once "$BASE_DIR/siro/formularios/permissao/periodoSolicitacoesInicio.php";
    require_once "$BASE_DIR/includes/rodape.php";
    exit();
}
else if( !EventoPeriodoLetivo::verificaEncerramentoInscricoes($periodoLetivo->getIdPeriodoLetivo()))
{
