...
function buscarTurmas($siglaCurso,$idPeriodoLetivo,$turno) {
    ...
    $filtroPeriodoLetivo = sprintf(" AND PL.`idPeriodoLetivo` = %s ", mysqli_real_escape_string($con, $idPeriodoLetivo));

    if(strtolower($turno) != 'todos'){
        $filtroTurno = sprintf(" AND T.`turno` = UPPER('%s') ", mysqli_real_escape_string($con, $turno));
    }
    ...
    while ($infoTurma = mysqli_fetch_array($result)) {
...
function buscarDisciplinasNaoOfertadas($siglaCurso,$idPeriodoLetivo, $turno) {
    ...
    $query = sprintf("SELECT 
        ...
        ORDER BY CC.periodo, CC.siglaDisciplina", 
            mysqli_real_escape_string($con, $idPeriodoLetivo),
            mysqli_real_escape_string($con, strtoupper($turno)),
