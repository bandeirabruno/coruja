...
class Turma {
    ...
    public function getInscricoesDePauta() {
        ...
        while( $reg = mysqli_fetch_array($result) ) {
            $inscricoes[] = Inscricao::getInscricao($this->getIdTurma(), $reg["matriculaAluno"] );
        }
        return $inscricoes;
    }
    ...
    public static function obterTurmasConfirmadasPorProfessor(Professor $professor) {        
        ...
        while( $resTurmas = mysqli_fetch_array($result) ) {
  ...
  /*Funcao que obtem turmas dada um determinado parametro
   ...
   public static function obterTurmas($condicao) {
       ...
       $con = BD::conectar();
        ...
        while( $resTurmasLiberadas = mysqli_fetch_array($result) ) {
    ...
    public static function getTurmaById($idTurma) {
        ...
        $query=sprintf("select * ".
                "from Turma ".
                "where idTurma = %d ",$idTurma);
        $result=BD::mysqli_query($query,$con);
       
        $turma = null;
        while( $resTurma = mysqli_fetch_array($result) ) {
    ...
    public function getAlunosBySituacao($situacao) {
        ...
        $queryListaAlunos = sprintf("SELECT i.matriculaAluno, p.nome, c.siglaDisciplina,
                        ...
                        mysqli_real_escape_string($con, $this->getIdTurma()),
                        mysqli_real_escape_string($con, $situacao)
        );
    ...
    private function obterDiasSemanaComAula() {
        ...
        $query = sprintf("select distinct TS.diaSemana from TempoSemanal TS  
            ...
            where T.idTurma = %d",
        mysqli_real_escape_string($con, $this->idTurma));
        $result = BD::mysqli_query($query, $con);
        $col = array();
        while( $reg = mysqli_fetch_array($result) ) {
            $col[] = $reg["diaSemana"];
    ...
    private function isDiaLetivoTurmaLiberado(DateTime $dataDiaLetivo) {
        ...
        $result = BD::mysqli_query($query, $con);
        if(BD::mysqli_result($result, 0, 0) == 0) return false;
        return true;
    }
    ...
    public function getDataLiberacaoNotas(ItemCriterioAvaliacao $itemCriterioAvaliacao) {
        ...
        return Util::converteDateTime(BD::mysqli_result($result, 0, 0));
    }
    ...
    public function isProfessorJaLiberouAlgumApontamento() {
        ...
        $resultDiaLetivo = BD::mysqli_query($queryDiaLetivo, $con);
        if(BD::mysqli_result($resultDiaLetivo, 0, 0) == 0) {
            $queryAvaliacao = sprintf("select count(*) from DiaLetivoTurma where "
                . "idTurma = %d and dataLiberacao is not null", $this->idTurma);
            $resultAvaliacao = BD::mysqli_query($queryAvaliacao, $con);
            if(BD::mysqli_result($resultAvaliacao, 0,  0) == 0 ) {
                return false;
            } else {
                return true;
            }
