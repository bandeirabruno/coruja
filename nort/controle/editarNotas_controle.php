...
switch ($acao) {
    ...
    case 'salvarDadosAJAX':
        ...
        if($insc_situacao == Inscricao::ID ) {
            ...
            $escaped_parecerInscricao = "'".mysqli_real_escape_string($con, $insc_parecer)."'";
        } else {
        ...
        $query = sprintf("" .
                        ...
                        mysqli_real_escape_string($con, $insc_situacao),    // #1
                        $escaped_mediaFinal,                         // #2
                        $escaped_totalFaltas,                        // #3
                        $escaped_parecerInscricao,                   // #4
                        mysqli_real_escape_string($con, $idTurma),          // #5
                        mysqli_real_escape_string($con, $numMatriculaAluno) // #6
        );
        BD::mysqli_query($query, $con);
        if (mysqli_errno($con) != 0) {
            exit("Erro MySql: ".mysqli_errno($con)." - ".mysqli_error($con));
        }
