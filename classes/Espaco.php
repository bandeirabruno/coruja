...
class Espaco { 
    ...
    public function __construct($idEspaco = null, $nome = null, $capacidade = null) {
...
//        $result = BD::mysqli_query($query1, $con);
//        if (!$result) {
...
//        }
    ...
    public static function incluirEspaco($nome, $capacidade, $con) {
        ...
        $resultMaxId=BD::mysqli_query($queryMaxId,$con);
        if(!$resultMaxId) {
          throw new Exception("Erro ao obter id espaço");
        }
        $idEspacoNovo=BD::mysqli_result($resultMaxId, 0, 0)+1;

        $query=sprintf("insert into Espaco (idEspaco, nome, capacidade)
            values(%d,'%s',%d)",
            $idEspacoNovo,
            mysqli_real_escape_string($con, $nome), $capacidade);
        $result=BD::mysqli_query($query,$con);
        if(!$result) {
    ...
    public function espacoEstaDisponivel(TempoSemanal $tempoSemanal,
        ...
        if(BD::mysqli_result($result, 0,0)==1) return false;
        else return true;
    }
