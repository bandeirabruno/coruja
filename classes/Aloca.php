...
class Aloca {
    ...
    public function __construct($idTurma = null, $idTempoSemanal = null, $idEspaco = null) {
    ...
    public function pega_aloca( $idTurma ) {
        ...
        $result = BD::mysqli_query( sprintf("SELECT * FROM Aloca as a ".
                                                "INNER JOIN TempoSemanal as t ".
                                                "ON a.idTempoSemanal = t.idTempoSemanal ".
                                                "INNER JOIN Espaco as e ".
                                                "ON a.idEspaco = e.idEspaco ".
                                                "WHERE a.idTurma = '%s'", mysqli_real_escape_string($con, $idTurma)), $con);

        // cria novo objeto
        $_resultSet = mysqli_fetch_array( $result);
        $__obj = new Aloca();
    ...
    public function lista_aloca( $conditionalStatement = '' ) {
         ...
         if(!empty($conditionalStatement)) { 
              $sqlStatement = sprintf("SELECT * FROM Aloca as a ".
                                ...
                                "INNER JOIN Espaco as e ".
                                "ON a.idEspaco = e.idEspaco ".
                                "%s", mysqli_real_escape_string($con, $conditionalStatement));
    ...
    public function obterTemposByTurma($idTurma){
        ...
        $query=sprintf("SELECT idTempoSemanal FROM Aloca ".
                       "WHERE idTurma = '%s'", mysqli_real_escape_string($con, $idTurma));
        
        $result=BD::mysqli_query($query,$con);

        while( $resTempos = mysqli_fetch_array($result) ) {
    ...
    public static function getListAlocaByIdTurma($idTurma) {
        ...
        $result = BD::mysqli_query($query,$con);
        $collection = array();
        while( $resAloca = mysqli_fetch_array($result) ) {
