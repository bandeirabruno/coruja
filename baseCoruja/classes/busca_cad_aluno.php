...
class buscaCadAluno
{
    ...
    // VERIFICA SE A PESSOA POSSUI MAIS DE UMA MATRICULA
        ...
        $res_checaMatricula = BD::mysqli_query($checaMatricula,$con) or die(mysqli_error($con));

            $this->quant_matriculas = mysqli_num_rows($res_checaMatricula);
         
    // EXECUTA O QUERY
        $resPessoa = BD::mysqli_query($sqlPessoa,$con) or die(mysqli_error($con));

        $rowPessoa = mysqli_fetch_array($resPessoa);
...
*/
    ...
    $this->opcao = $opcao; 
        ...
        */
        ...
        $resMat = BD::mysqli_query($sqlMat,$con) or die(mysqli_error($con));
    
        $nmat=1;   
        while($rowMat = mysqli_fetch_array($resMat))
        {
