...
switch( $acao) 
{
        ...
        if( $siglaCurso !== NULL)
        {
            ...
            if( filter_input( INPUT_POST, "idPeriodoLetivo", FILTER_SANITIZE_NUMBER_INT) !== NULL) 
            {
                ...
                $query = sprintf("".
                    ...
                    mysqli_real_escape_string($con, $_POST['siglaCurso']),
                ...
                $query = sprintf(
                    ...
                    $_POST['idPeriodoLetivo']);
                $result=BD::mysqli_query($query,$con);
                while( $resPro = mysqli_fetch_array($result) ) {
    ...
    case 'buscarTurmasResultado':
        ...
        if ($_POST['idPeriodoLetivo'])
            $filtroPeriodoLetivo = ' AND PL.`idPeriodoLetivo` = ' . mysqli_real_escape_string($con, $_POST['idPeriodoLetivo']) . ' ';
        else
            $filtroPeriodoLetivo = '';

        // CURSO
        if ($_POST['siglaCurso'])
            $filtroCurso = ' AND T.`siglaCurso` = "' . mysqli_real_escape_string($con, $_POST['siglaCurso']) . '" ';
        else
            $filtroCurso = '';

        // DISCIPLINA
        if ($_POST['siglaDisciplina'])
            $filtroDisciplina = ' AND T.`siglaDisciplina` = "' . mysqli_real_escape_string($con, $_POST['siglaDisciplina']) . '" ';
        else
            $filtroDisciplina = '';

        // PROFESSOR
        if ($_POST['idPessoaProfessor'])
            $filtroProfessor = 'AND MP.`idPessoa` = ' . mysqli_real_escape_string($con, $_POST['idPessoaProfessor']) . ' ';
        else
            $filtroProfessor = '';
...
function registrarLog($turmas) {
    ...
    foreach ($turmas as $idTurma) {
        ...
        $result = BD::mysqli_query($SQL_PERIODO_LETIVO);
        ...
        $result = BD::mysqli_query($SQL_PERIODO);
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
        ...
        $result = BD::mysqli_query($SQL_NOME_DO_PROFESSOR);
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
        ...
        $SQL_NOME_DISCIPLINA = 'select CC.`siglaDisciplina`, CC.`nomeDisciplina` '
                ...
                . 'where T.`idTurma` = ' . $idTurma . ' '
                . 'and T.siglaDisciplina = CC.`siglaDisciplina`';
        $result = BD::mysqli_query($SQL_NOME_DISCIPLINA);
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
        ...
        $result = BD::mysqli_query($SQL_TURNO_DA_TURMA);
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
            $turno = $row['turno'];
...
and CC.`siglaDisciplina` = T.`siglaDisciplina`';
            ...
            $result = BD::mysqli_query($SQL_CREDITOS);
            while($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
