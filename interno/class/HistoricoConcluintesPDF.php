...
class HistoricoConcluintesPDF extends FPDF {
    ...
    public function criarXML($dtini,$dtfim,$estabelecimentoVestibular, $chtda,$ches,$chaec,$tchc,
       ...
       // Para salvar o arquivo, descomente a linha
        $dom->save( $_SERVER['DOCUMENT_ROOT'].$RAIZ_CORUJA.Config::DIR_AREA_DADOS . "/" . $_REQUEST['mat'] . ".xml");
    }

    public function obterDadoAluno($matriculaAluno)
    {
        $con = BD::conectar();
        $result = BD::mysqli_query(sprintf("SELECT
     ...
     MONTH(PeriodoLetivo.dataInicio) as mesInicio,
     YEAR(PeriodoLetivo.dataInicio) as anoInicio,
...
WHERE
     MatriculaAluno.matriculaAluno = '%s'", mysqli_real_escape_string($con, $matriculaAluno)), $con);

        $objetos = array();
        if (mysqli_num_rows($result) > 0) {
			
            //solicitação 01
            //Solicitação da Jamile em 17/09/2019, pedido por Ricardo Marciano
            //Troca da sigla do período letivo por mês(nome) e ano: 2010.2 => Agosto/2010
            ...
            while ($rs = mysqli_fetch_array($result)) {
                ...
                $HistoricoConcluintesPDF->setSiglaPeriodoLetivo($meses[$rs['mesInicio']]."/".$rs['anoInicio']);
    ...
    public function gerarCabecalho($matrizId) {
        ...
        if ($matrizId < 6){ //Matriz Antiga
            ...
            $txt = "CURSO SUPERIOR DE TECNOLOGIA EM ANÁLISE E DESENVOLVIMENTO DE SISTEMAS";
        }
        else{ // Matriz Nova
        ...
        if ($matrizId < 6){ //Matriz Antiga
            ...
            RECONHECIMENTO:PARECER C.E.E NÚMERO 066/2009 DE 09/06/2009 D.O.E.R.J. 14/07/2009
            RENOVAÇÃO DE RECONHECIMENTO: PARECER C.E.E 100 DE 11/12/2018
            HOMOLOGADO PELA PORTARIA C.E.E NÚMERO 3710 DE 25/02/2019 D.O.E.R.J. 28/02/2019
            RECREDENCIAMENTO: PARECER C.E.E NÚMERO 018 DE 19/03/2019
            HOMOLOGADO PELA PORTARIA C.E.E NÚMERO 3716 DE 26/04/2019 D.O.E.R.J 30/04/2019';
        }
        else{ // Matriz Nova
            $txt = 'DECRETO DE CRIAÇÃO NÚMERO 30.938 DE 18/03/2002 D.O.E.R.J. 19/03/2002
            RECONHECIMENTO:PARECER C.E.E NÚMERO 066/2009 DE 09/06/2009 D.O.E.R.J. 14/07/2009
    ...
    public function obterDescricaoDoAluno($matriculaAluno) {
        ...
        $MatriculaAluno1 = MatriculaAluno::obterMatriculaAluno($matriculaAluno);
        //$matricula = MatriculaAluno::obterMatriculaAluno($matriculaAlunoString);
        $matriz = $MatriculaAluno1->getMatrizCurricular();
        $matrizId = $matriz->getIdMatriz();
    ...
    public function obterDadosGradeCurricular($matriculaAluno) {
        ...
        //$matricula = MatriculaAluno::obterMatriculaAluno($matriculaAlunoString);
        $matriz = $MatriculaAluno->getMatrizCurricular();
        $matrizId = $matriz->getIdMatriz();
        ...
        $this->Cell($cumprimento2, $espacamentoHorizontal, $txt, ($this->debug) ? 1 : '', 0, $alinhamento);
    ...
    public function gradeCurricular($matriculaAlunoString) {
        ...
        foreach ($acc as $cc) {
            ...
            if ( $quitacaoTO == null && $cc->getTipoComponenteCurricular() == "OBRIGATÓRIA") {
                unset($_SESSION['arr']);
                $_SESSION['arr'] = array();
