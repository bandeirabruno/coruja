...
class Curso {
	...
	order by MA.matriculaAluno",
        ...
        mysqli_escape_string($con, $dataInicioUltPeriodoLetivo));
    ...
    */
    public static function obterCurso( $siglaCurso) 
    {
        ...
        if(mysqli_num_rows($result) > 0) { //Valida se algum Curso foi encontrado no sistema.
            $resCurso = mysqli_fetch_array($result); //Obtem o resultado do banco
            $curso = new Curso();
    ...
    public static function obterCursosOrdemPorSigla() 
    {
        ...
        while ($__rs = mysqli_fetch_array($result)) 
        {
            $__newObj = new Curso();
    ...
    public static function alterarCurso($siglaCursoAntes,$siglaCursoDepois, $nomeCurso, $idTipoCurso, $con){
        ...
        where siglaCurso='%s'",
            mysqli_real_escape_string($con, $siglaCursoDepois),
            mysqli_real_escape_string($con, $nomeCurso),
            $idTipoCurso,
            mysqli_real_escape_string($con, $siglaCursoAntes));
        $result=BD::mysqli_query($query,$con);
        if(!$result) {
    ...
    public static function excluirCurso(Curso $Curso, $con) {
        ...
        $query2 = sprintf("delete from Curso where siglaCurso='%s'",
                        $Curso->getSiglaCurso());
        $result2 = BD::mysqli_query($query2, $con);
        if (!$result2) {
    ...
    public static function incluirCurso($siglaCurso, $nomeCurso, $idTipoCurso, $con) 
    {
        ...
        $query=sprintf("insert into Curso (siglaCurso, nomeCurso, idTipoCurso)
            values('%s','%s',%d)",
            mysqli_real_escape_string($con, $siglaCurso), mysqli_real_escape_string($con, $nomeCurso),
                $idTipoCurso);
        $result=BD::mysqli_query($query,$con);
        if(!$result) {
